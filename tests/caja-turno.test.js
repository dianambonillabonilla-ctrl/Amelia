...
(function () {
  ...
  exigirFalla(env, p({
    ...
    item: { fecha: HOY, sede: SEDE, tipo: 'Otro ingreso', valor: 5000, motivo: 'Hielo' }
  }), 'registrar un movimiento sin haber abierto la caja');

  exigirFalla(env, p({
    action: 'caja_cerrar',
    item: { fecha: HOY, sede: SEDE, efectivo_contado: 100000, caja_fuerte_contada: 0 }
  }), 'cerrar una caja que nunca se abrió');

  const abrir = exigir(env, p({ action: 'caja_abrir', item: { fecha: HOY, sede: SEDE, base_inicial: 50000, caja_fuerte_inicial: 0, observacion_apertura: 'primera apertura, sin cierre anterior' } }), 'abrir la caja');
  ...
  const reabrir = exigir(env, p({ action: 'caja_abrir', item: { fecha: HOY, sede: SEDE, base_inicial: 99999, caja_fuerte_inicial: 0 } }), 'reabrir el mismo día');
...
(function () {
  ...
  exigir(env, p({ action: 'caja_abrir', item: { fecha: HOY, sede: SEDE, base_inicial: 50000, caja_fuerte_inicial: 0, observacion_apertura: 'primera apertura, sin cierre anterior' } }), 'abrir la caja');
  ...
  // 'Gasto' se retiró de Caja (ago 2026, Diana): los gastos se manejan en FUDO. 'Otro ingreso' es
  // el otro tipo que no restan de la caja operativa por un tope de disponibilidad.
  exigir(env, p({
    action: 'caja_movimiento_registrar',
    item: { fecha: HOY, sede: SEDE, tipo: 'Otro ingreso', valor: 3000, motivo: 'Venta de bolsa de hielo sobrante' }
  }), 'registrar otro ingreso');

  // El esperado ya refleja los movimientos ANTES de cualquier cierre — esto es lo que antes
  ...
  assert.equal(estado.efectivo_esperado, 50000 - 20000 + 3000, 'base - entregas + otros ingresos');
...
(function () {
  ...
  // Esperado = 50000 (base) - 20000 (entrega) = 30000 (sin ventas de FUDO en la prueba). La base
  // siguiente no puede ser mayor que lo contado, así que se deja una parte como base y el resto se
  // entrega (regla vigente desde antes de esta sesión, ver cajaCerrar_).
  const cierreExacto = exigir(env, p({
    action: 'caja_cerrar',
    item: { fecha: HOY, sede: SEDE, efectivo_contado: 30000, caja_fuerte_contada: 0, base_siguiente: 20000 }
  ...
  // cajaCerrar_ no devuelve entrega_cierre directamente (queda en la fila guardada) — se confirma
  // consultando el estado, que es como lo lee caja.html después de cerrar.
  const estadoTrasCierre = exigir(env, p({ action: 'caja_estado', fecha: HOY, sede: SEDE }), 'ver el estado tras cerrar');
  assert.equal(estadoTrasCierre.apertura.entrega_cierre, 30000 - 20000, 'lo que sobra sobre la base siguiente es lo que se entrega');

  // Cerrar de nuevo el mismo día no debe recalcular ni duplicar.
  const yaCerrado = exigir(env, p({
    action: 'caja_cerrar',
    item: { fecha: HOY, sede: SEDE, efectivo_contado: 999999, caja_fuerte_contada: 0 }
...
(function () {
  ...
  // Faltante: se contaron 45000 pero se esperaban 50000 (sin movimientos ni ventas). Una diferencia
  // ya no exige Administrador (Diana, ago 2026), pero sí una observación explicando qué pasó.
  const faltante = exigir(env, p({
    action: 'caja_cerrar',
    item: { fecha: HOY, sede: SEDE, efectivo_contado: 45000, caja_fuerte_contada: 0, base_siguiente: 45000, observacion: 'faltan 5.000, se investiga' }
  }), 'cerrar con faltante');
...
(function () {
  ...
  // Nota: el router (Code.gs) ya bloquea a Cocina de todas las rutas caja_* (ago 2026, Diana), pero
  // estas funciones de más bajo nivel (sedeEscrituraPermitida_/cajaPuedeCerrar_) no dependen de eso
  // — se prueban aquí directamente con un rol que no sea Administrador/Encargado, igual que antes.
  const propiaSede = cajaAbrir_({ fecha: HOY, sede: SEDE, base_inicial: 0, caja_fuerte_inicial: 0 }, usuarioCocinaSA);
