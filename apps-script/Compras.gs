...
function compraGuardar_(factura, lineas, usuario) {
  ...
  lock.waitLock(30000);
  try {
    return compraGuardarInterna_(factura, lineas, usuario);
  } finally {
    lock.releaseLock();
  }
}

function compraGuardarInterna_(factura, lineas, usuario) {
  factura = factura || {};
  lineas = lineas || [];
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    const validado = validarItemInventario_(l, 'producto');
  ...
  if (!isFinite(subtotal) || subtotal < 0 || !isFinite(impuestos) || impuestos < 0 || !isFinite(total) || total < 0) {
    return { ok: false, error: 'Subtotal, impuestos y total deben ser valores no negativos' };
  }
  if (Math.abs(subtotal - totalLineas) > 0.01) {
    return { ok: false, error: 'El subtotal debe coincidir con la suma de las líneas (' + totalLineas.toFixed(2) + ')' };
  }
  if (Math.abs(total - (subtotal + impuestos)) > 0.01) {
    return { ok: false, error: 'El total debe ser igual a subtotal más impuestos (' + (subtotal + impuestos).toFixed(2) + ')' };
  }
  const numeroFactura = String(factura.numero_factura || '').trim();
  if (numeroFactura) {
    const proveedor = normalizar_(factura.proveedor);
    const duplicada = leerTabla_(SHEET_NAMES.COMPRAS_FACTURAS).some(function (f) {
      return normalizar_(f.proveedor) === proveedor && String(f.numero_factura || '').trim() === numeroFactura;
    });
    if (duplicada) return { ok: false, error: 'Ya existe una factura de este proveedor con el número ' + numeroFactura };
  }
