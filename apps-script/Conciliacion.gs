...
function conciliarComidaPorSede_(fecha, sedesPermitidas) {
  ...
  const sedes = sedesPermitidas || ['Centro de Producción', 'San Antonio', 'Capri'];
  const resultado = {};
...
function trasladosNetosPorItem_(fecha, sede, indice) {
  ...
  leerTabla_(SHEET_NAMES.TRASLADOS).filter(function (t) {
    ...
    const fechaEnvio = formatearFecha_(t.fecha);
    return ['Confirmado', 'Resuelto'].indexOf(t.estado) !== -1 &&
      ((t.sede_origen === sede && fechaEnvio === fecha) || (t.sede_destino === sede && fechaRecepcion === fecha));
  }).forEach(function (t) {
