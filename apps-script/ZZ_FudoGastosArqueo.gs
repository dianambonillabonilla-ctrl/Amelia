 ...
 *
 * Para Caja DILANA solo importa el EFECTIVO físico que salió de una caja FUDO. Por eso se guarda
 * todo pago detectado, pero el cálculo resta únicamente filas no canceladas, asociadas a una caja
 * registradora y cuyo medio de pago sea efectivo. Una transferencia/tarjeta nunca reduce el cajón.
 */
 ...
 * El modelo legado (sin payments resueltos) usa amount/useInCashCount/paymentDate del Expense.
 */
function fudoGastosArqueoFilasDesdeExpense_(expense, incluidos, fechaDesde, fechaHasta) {
  ...
  pagosPtr.forEach(function (ptr) {
    ...
    // En el modelo de pagos múltiples, FUDO asocia cashRegister al pago cuando ese pago impacta un
    // arqueo. También se acepta useInCashCount si alguna versión futura lo expone en Payment.
    const usaArqueo = !!cajaPtr || fudoGastosBool_(pa.useInCashCount);
  ...
  // Compatibilidad con cuentas / registros donde Expense conserva el pago único directamente.
  if (pagosResueltos === 0) {
    ...
    if (fudoGastosFechaEnRango_(momento, fechaDesde, fechaHasta)) {
      ...
      filas.push({
        ...
        id_gasto: String(expense.id),
        fecha_pago: formatearFecha_(momento),
        // paymentDate/date pueden ser solo fecha. Si no traen hora se deja momento_fudo vacío y
        // primera_sincronizacion_en determina si ya existía antes de abrir DILANA.
        momento_fudo: /T\d{2}:\d{2}/.test(String(momento)) ? new Date(momento) : '',
        monto: Number(ea.amount) || 0,
        cancelado: canceladoGasto,
        usa_arqueo: fudoGastosBool_(ea.useInCashCount) && !!cajaPtr,
        es_efectivo: fudoGastosEsEfectivo_(metodo),
        metodo_pago: metodo.nombre,
        metodo_tipo: metodo.tipo,
        caja_fudo: cajaNombre,
...
function fudoGastosArqueoConsultar_(fechaDesde, fechaHasta) {
  ...
  // Consulta A: necesaria para el modelo legado, donde el pago vive directamente en Expense.
  ...
  // Consulta B: necesaria para una factura/gasto antiguo que se PAGÓ dentro del rango actual.
...
function fudoGastosArqueoUpsert_(filas, usuario) {
  ...
  filas.forEach(function (f) {
    ...
    if (existente) {
      // primera_sincronizacion_en es inmutable: permite saber si un gasto sin hora ya existía antes
      // de abrir Caja o apareció durante el turno.
...
function fudoApiSincronizarGastosArqueo_(fechaDesde, fechaHasta, usuario) {
  ...
  const res = Object.assign({}, upsert, {
    tipo:'gastos_arqueo',
    movimientos_encontrados:filas.length,
    impactan_arqueo_efectivo:impactan,
    monto_arqueo_efectivo:Number(monto.toFixed(2))
  });
  if (typeof fudoApiSyncRegistrar_ === 'function') {
    fudoApiSyncRegistrar_('gastos_arqueo', {
      ok:true, fecha_desde:fechaDesde, fecha_hasta:fechaHasta,
      usuario:usuario && usuario.nombre, importados:upsert.importados,
      actualizados:upsert.actualizados, movimientos_encontrados:filas.length,
      monto_arqueo_efectivo:res.monto_arqueo_efectivo, error:''
    });
  }
  return res;
}
...
/**
 * Total atribuible al turno DILANA. Para pagos con timestamp FUDO se usa ese instante. Para el
 * modelo legado de solo fecha se usa cuándo DILANA vio el registro por primera vez.
 */
